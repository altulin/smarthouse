...
import { emoji, keyboards } from "./modules/keyboard.js";
import { getForecast, line} from "./modules/weather.js";
import { createTasksSunPositions } from "./modules/crontab.js";
// import { readFileSync } from "fs";
import { getCounterMsg } from "./modules/counter.js"
import { createExerisesPlan } from "./modules/bodybuilding.js"
import { reply, replyGuest } from "./modules/reply.js"
...
class SimpleSendMessage extends SendMessage {
	constructor(id, msg) {
		super(id);
		this._msg = msg;
	}
  getMarkup() {
    return reply.get(this._msg)[1];
	}
	getMessage() {
		return	`${this._msg}${reply.get(this._msg)[0]}`;
	}
}
	...
	bot.on(`message`, (msg) => {
		...
		if (users.toString().includes(chatId)) {
			...
			if (replyMsg) {
				...
				new SimpleSendMessage(chatId, textMsg).sendMessage()
			} else {
		...
		if (users.toString().includes(chatId)) {
			...
			if (textMsg.includes(`weather`)) {
				// console.log(getForecast())
					sendMessage(chatId, `Не знаю!`, keyboards.main);
